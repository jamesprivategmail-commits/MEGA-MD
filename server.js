...
// ---- Dynamic pairing route ----
// Lets you request a pairing code for ANY number on demand, instead of
// relying on a fixed OWNER_NUMBER / PAIRING_NUMBER env var.
// The actual pairing logic lives in index.js and is attached below via
// setPairingHandler(), since only index.js has access to the live socket.
...
app.get('/pair-request', async (req, res) => {
    ...
    if (!number) {
        ...
        <title>Nova Tech Pairing</title>
        <style>
            body{background:#0f172a;color:#fff;font-family:system-ui,sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0}
            .card{background:rgba(30,41,59,.7);padding:30px;border-radius:24px;width:90%;max-width:400px;text-align:center}
            h1{color:#25d366;letter-spacing:2px}
            input{width:90%;padding:12px;border-radius:10px;border:1px solid #334155;background:#0b1220;color:#fff;margin:10px 0;font-size:1rem}
            button{width:95%;padding:12px;border-radius:10px;border:none;background:#25d366;color:#062;font-weight:bold;font-size:1rem}
        </style></head><body>
        <div class="card">
            <h1>NOVA &middot; TECH</h1>
            <p>Enter your WhatsApp number to pair</p>
            <form>
                <input name="number" placeholder="2348012345678" required>
    ...
    if (!pairingHandler) {
        return res.status(503).send('Pairing system not ready yet. Try again in a few seconds.');
    }
    ...
    try {
        ...
        res.send(`
        <!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0">
        <style>body{background:#0f172a;color:#fff;font-family:system-ui,sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0}
        .card{background:rgba(30,41,59,.7);padding:30px;border-radius:24px;width:90%;max-width:400px;text-align:center}
        .code{font-size:2rem;letter-spacing:4px;color:#25d366;font-weight:bold;margin:20px 0}</style></head>
        <body><div class="card"><h1>NOVA &middot; TECH</h1><p>Your pairing code:</p>
        <div class="code">${code}</div>
        <p>WhatsApp &rarr; Linked Devices &rarr; Link with phone number</p></div></body></html>`);
    } catch (err) {
        res.status(400).send(`<p style="font-family:sans-serif;color:red">Error: ${err.message}</p>`);
    }
});
